(() => {
  ...
  const fields = (prefix = '') => `<div class="b2b-fields">
    ...
    <label hidden data-specification-input="${prefix}denier">Denier (D)<input disabled name="${prefix}denierValue" inputmode="decimal" type="text" placeholder="Enter denier"></label>
    ...
    <label hidden data-specification-input="${prefix}tpm">TPM<input disabled name="${prefix}tpmValue" inputmode="decimal" type="text" placeholder="Enter TPM"></label>
  ...
  const setupSpecificationInputs = (form, prefix = '') => {
    const specifications = [
      { mode: `${prefix}denier`, value: `${prefix}denierValue`, specify: 'Specify Denier' },
      { mode: `${prefix}tpm`, value: `${prefix}tpmValue`, specify: 'Specify TPM' }
    ];
    const sync = () => specifications.forEach(({ mode, value, specify }) => {
      const selector = form.elements[mode]; const input = form.elements[value]; const wrapper = input?.closest('[data-specification-input]');
      if (!selector || !input || !wrapper) return;
      const isSpecified = selector.value === specify;
      wrapper.hidden = !isSpecified;
      input.disabled = !isSpecified;
      input.required = isSpecified;
      if (!isSpecified) input.value = '';
    });
    specifications.forEach(({ mode }) => form.elements[mode]?.addEventListener('change', sync));
    sync();
    return sync;
  };
  ...
  if (product) {
    ...
    const form = section.querySelector('form'); setupSpecificationInputs(form); const wa = section.querySelector('[data-whatsapp]'); const refresh = () => wa.href = whatsapp(encodeConfig(form), 'quote'); form.addEventListener('input', refresh); form.addEventListener('change', refresh); refresh();
  ...
  if (isQuote || isSample) {
    ...
    const form = formSection.querySelector('form'); Object.entries(stored).forEach(([key, value]) => { const control = form.elements[key]; if (control && value) control.value = value; }); setupSpecificationInputs(form);
