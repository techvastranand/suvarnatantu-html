...
const requiredCanonicalPaths=['/','/products/','/metallic-yarn/','/zari-yarn/','/zari-yarn/imitation-zari/','/applications/','/zari-lab/','/manufacturing/','/industries/','/export/','/specifications/','/samples/','/about-us/','/contact/','/request-quote/','/privacy-policy/','/terms/','/blog/','/blog/articles/'];
...
for(const path of [...requiredCanonicalPaths,...categories.map(slug=>`/blog/category/${slug}/`),...manifest.articles.map(slug=>`/blog/${slug}/`)])if(!urls.includes(site+path))throw Error(`Sitemap is missing ${path}`);
...
for(const [html,canonical,path] of [[productsHome,`${site}/products/`,'products/index.html'],[blogHome,`${site}/blog/`,'blog/index.html']])if((html.match(new RegExp(`rel="canonical" href="${canonical}"`,'g'))||[]).length!==1)throw Error(`${path} must claim its canonical URL exactly once.`);
...
for(const slug of manifest.articles){const html=await readFile(resolve(root,'blog',slug,'index.html'),'utf8');if((html.match(/<h1[ >]/g)||[]).length!==1)throw Error(`${slug} does not have exactly one H1.`);if(!html.includes(`rel="canonical" href="${site}/blog/${slug}/"`))throw Error(`${slug} is missing its canonical URL.`);const category=/<a class="kc-tag" href="\/blog\/category\/([^/]+)\/">/.exec(html)?.[1];if(!categories.includes(category))throw Error(`${slug} is missing a recognized primary category.`);const categoryPage=await readFile(resolve(root,'blog','category',category,'index.html'),'utf8');for(const page of [categoryPage,...(archive?[archive]:[])])if(!page.includes(`href="/blog/${slug}/"`))throw Error(`${slug} is orphaned from blog discovery pages.`);}
